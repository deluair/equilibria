"""Shared fixtures for L7 Financial layer tests."""
...
async def test_db(tmp_path, monkeypatch):
    db_path = str(tmp_path / "test_financial.db")
...
async def _insert_series(db_conn, source: str, series_id: str, country: str,
    ...
    await db_conn.execute(
        """
        INSERT OR IGNORE INTO data_series (source, series_id, country_iso3, name, description)
        VALUES (?, ?, ?, ?, ?)
        """,
        (source, series_id, country, description, description),
    )
