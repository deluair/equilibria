import pytest
from app.layers.trade.market_diversification import MarketDiversification


def test_instantiation():
    model = MarketDiversification()
...
def test_name():
    model = MarketDiversification()
    assert model.name == "Market Diversification"
...
async def test_run_with_year_kwarg(db_conn):
    model = MarketDiversification()
    result = await model.run(db_conn, country_iso3="BGD", year=2021)
    assert isinstance(result, dict)
    assert "score" in result
