import pytest
from app.layers.trade.trade_creation import TradeCreation
...
def test_name():
    model = TradeCreation()
    assert model.name == "Trade Creation & Diversion"
...
async def test_compute_with_year_kwarg(db_conn):
    model = TradeCreation()
    result = await model.compute(db_conn, country_iso3="BGD", year=2018)
    assert isinstance(result, dict)
    assert "score" in result
